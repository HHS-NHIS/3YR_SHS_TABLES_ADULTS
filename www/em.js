...
$(document).ready(function () {
    ...
    */
    ...
    $(document).on('updateValues', function () {
        ...
        is_quarterly = year.includes("2019") || year.includes("2020") || year.includes("2021") || year.includes("2022");
        ...
        if (is_quarterly) {
            ...
            q_range = range(q_yearStart, q_year, by = 0);
    ...
    $(document).on('updatePlot', function () {
        ...
        if (isTrend) {
            ...
            y_line = y_values.slice(0).toString().split(',').map(Number);
            y_max22 = Math.max.apply(null, y_line.filter(function (n) { return !isNaN(n); })) + 10;


                lcl_y_line = y_ses.slice(0, 1).toString().split(',').map(Number),
                ucl_y_line = y_ses.slice(-1).toString().split(',').map(Number),

                y_min = Math.min.apply(null, lcl_y_line.filter(function (n) { return !isNaN(n); })),
                y_max = Math.max.apply(null, ucl_y_line.filter(function (n) { return !isNaN(n); })) + 10,
            ...
            layout.yaxis = {
                autorange: 'reversed', //visible: !hideYaxis,
    ...
    $(document).on('updateNotes', function () {
        ...
        qnewtitle3 = qnewtitle2;


        if (colX == "education") {
            myFunction2(qnewtitle3);
        }
        else if (colX == "insurance_Under65") {
            myFunction3(qnewtitle3);
        }
        else if (colX == "insurance_65over") {
            myFunction4(qnewtitle3);
        }

        //var titleCaption = colX ? "education" :
        //qnewtitle.replace('aged 18', "aged 25");
        //$('#table-title').text(qnewtitle);


        //this is new part
        var titleCaption = showSEs ? qnewtitle3 :
            qnewtitle3.replace(" (95% confidence intervals)", "");
        $('#table-title').text(titleCaption);



      //  var titleCaption2 = showSEs ? qnewtitle3 :
       //     qnewtitle3.replace(" (95% confidence intervals)", "");
      //  $('#table-title2').text(titleCaption2);



        var plotCaption = showSEs ? qnewtitle3 :
            qnewtitle3.replace(" (95% confidence intervals)", "");
        ...
        //$('#table-title2').html(titleCaption2);
